...
const View = (props: Props) => {
  const [settingData, setSettingData] = useState<DocumentData>();
  const [labels, setLabels] = useState<string[]>([]);

  const [base64Text, setBase64Text] = useState<string>(imageCode);
  ...
  const getTemplate = async () => {
    ...
    setSettingData(settingDoc.data());
  };
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    props.changeIsLoading(true, "追加中です");

    const formEl: any = document.forms;
    const labelInput = formEl.addPerson.labelInput;
    const valueInput = formEl.addPerson.valueInput;
    const labelArray: string[] = [];
    const valueArray: string[] = [];

    let valueFlag = false;

    labelInput.forEach((label: any) => {
      labelArray.push(label.value);
    });
    valueInput.forEach((value: any) => {
      if (value.value) {
    ...
    valueArray.unshift(base64Text);

    setFirestore(props.fileName, labelArray, valueArray);
  };
    ...
    <div className={styles.wrapper}>
      <div className={styles.fileNameWrap}>
        <div
          ...
          onClick={() => props.changeInputState("fileName")}
        >
          <TfiArrowLeft className={styles.arrow} />
        </div>
        <p className={styles.fileName}>{props.fileName}</p>
      </div>
      <form onSubmit={handleSubmit} name="addPerson">
        ...
        {labels.map((label: string, i: number) => (
          <Input key={i} index={i} label={label} changeLabels={changeLabels} />
        ))}
